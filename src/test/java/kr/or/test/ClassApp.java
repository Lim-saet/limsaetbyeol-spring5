...
class Circle {
	...
	public Circle(int radius) {
	         r= radius; //Get|Set 중에 Set 저장 멤버변수 저장처리
}
	//원의넓이를 반환하는 메서드 반환리턴값의 크기가 더블형(아래)
...
//클래스 상속에 대한 연습(아래)
 ...
 abstract class GraphicObject {
	 ...
	 //추상메서드를 만드는 이유는 메서드가 수십개 수백개 일때 개발자 구현할때 어려움이 존재
	 //위 문제를 해소한 기능의 클래스  수백개의 메서드를 이름만 모아서, 유지보수가 편리하게 처리한 방식
 }
 //추상클래스를 사용하는 방법(아래)
 //스프링에서는 추상클래스보다는 인터페이스를 더 많이 사용, 사용의도는 비슷합니다
 class Traiangle extends GraphicObject{

	@Override //부모클래스의 메서드를 재정의 해서 사용 = 오버라이드 /오버로든 2개  
	void draw() {
		// 삼각형을 만듦
		System.out. println("  *  ");
		System.out. println(" * * ");
...
public class ClassApp {
	...
	public static void main(String[] args) {
		...
		GraphicObject traiangleObject = new Traiangle(); //추상클래스는 객체로 만들지 못하기 때문에
