...
public class Step1 {
	  public void Step1() {
		  // 클래스명과 같은이름의 메서드를 생성자매서드 자동으로 만들어집니다.
		  //중요한이유는 다른 클래스에서 Step1을 객체로 만들때 Step1 aaa = new Step1();
		  //자바앱에서는 객체를 생성자 메서드로 만들때 필수
		  //단, 스프링에선 @Injext로 객체를 만들어서 사용
	  }
     //멤버변수(전역변수)는 Step1클래스에 영향을 모두 주는 변수 
	 //내부변수(내부변수)는 main메서드 내부에서만 영향을 주는 변수 
//	 private String name; //멤버변수 사용예,step2클래스에서 사용예정
	public static void main(String[] args) {
		...
		//변수에 값을 입력(아래)
		name = "홍길동"; // 문자이기 때문에 ""
		age = 10;//자바스크립트 보다는 자료형 사용에 엄격
		phoneNum = "000-0000-000";//1문장 끝을 명시 		
		...
		//메서드명이 같음. 단, 로드된 파라미터가 다른 메서드를 호출=오버로드 (loading...)
	}
	...
	private static void printMember(String names, int ages, String phoneNums) {
		...
		System.out.println("입력하신 회원의 이름은 "+names+"님, 나이는 "+ages+", 폰번호는 "+phoneNums);
