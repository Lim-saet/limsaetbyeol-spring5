      ...
      <div id="container">
        ...
        <div class="bodytext_area box_inner">
          <ul class="bbsview_list">
            ...
            <span><fmt:formatDate pattern="yyyy-MM-dd hh:mm:ss" value="${boardVO.reg_date}" /></span></li>
            ...
            <c:forEach begin="0" end="1" var="idx">
            	...
            	<!-- 위 처럼 c:url로 쿼리스트링을 처리하면 한글이 인코딩되어서 전송됌 -->
            		<a href="${url}">다운로드
              ...
              <div class="collapse timeline" id="collapseReply">
                ...
                <div class="row">
                  <ul class="pagination" style="margin: 0 auto" ;>
					...
					<a href="#" aria-controls="example2" data-dt-idx="0" tabindex="0" class="page-link">Previous
					</a>
				</li>
...
var replyList = function() {
	...
	$.ajax({
		...
		//console.log(result);//크롬에서 확인가능할때
		//전체json데이터를 출력할때 stringify함수로 형변환해서 출력
		//만약에 json데이터에서 키이름을 알게되면, stringfy함수필요없이 result.responseText 출력가능
...
$(document).ready(function(){
	...
	$("#btn_reply_write").click(function(){
		...
		$.ajax({
			...
			beforeSend:function() {
				//alert("전송 전에 실행할 내용출력1");
			},
			complete:function() {
				//success 완료된 이후 실행해야할 내용이 있을때 사용 
				//suuccess 에서 함수가 여러개 실행될때 순서가 중요한 경우 분리해서 사용
				$("#reply_text").val("");//기존 입력창의 댓글 내용을 지우기
				alert("등록이 완료되었습니다.");
			},
			async:true
			//Async+Javascript And Xml:비동기 데이터통신
			//Ajax이지만 비동기 false로 하는경우(아이작스로 대용량 첨부파일을 업로드할때)
			//success:업로딩중 ...표시가됨 -> 업로드가 완료되면 complete실행됌
