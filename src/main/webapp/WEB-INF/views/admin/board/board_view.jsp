            ...
            <div class="card card-primary">
              ...
              >
                  ...
                  <div class="form-group">
                    ...
                    <c:if test="${boarVO.save_file_names[idx] != null}">
                    <div class="input-group">
                      <div class="custom-file">
                      <!-- 첨부파일을 URL로 직접접근하지 못하기 때문에 컨트롤러로만 접근이 가능(다운로드전용 메서드 생성)-->
                      ...
                      <!-- jstl에서 변수 사용하기 fn.split('데이터','분할구분값') 목적: 확장자를 이용해서 이미지 미리보기를 할 건지 결정 img태그 사용
                      		String fileNameArray = String.split('변수값','분할기준');
                      -->
                      <c:set var="fileNameArray" value="${fn:split(boardVO.save_dile_names[idx],'.')}" />
                      ...
                      <!-- 그림판.얼굴.jpg 파일을 위 extName = fileNameArray[2] = jpg  -->
						...
						<!-- containsIgnoreCase('비교할구문','비교기준값') -->
						<c:choose>
						<c:when test="${fn:containsIgnoreCase(checkImgArray,extName)}">
						<img src="/img_preview?save_file_name=${boardVO.save_file_names[idx]}" style="width:100%;">
						</c:when>
						<c:otherwise>
						<!-- 아무의미 없이 개발연습용으로 -->
