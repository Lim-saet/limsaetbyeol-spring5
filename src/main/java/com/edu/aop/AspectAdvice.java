...
public class AspectAdvice {
	...
	//아래 조인포인트(진입점)은 board_delete || board_update* 메서드를 실행할때, 본인이 작성한 글인지 확인하는 기능을 추가 (단, ROLE_ADMIN 사용자는 제외)
	...
	public Object check_board_crud(ProceedingJoinPoint pjp) throws Throwable {
		//request 객체는 이전페이지 URL+session_userid(세션값)을 가져오기 위해서 필요
		HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.currentRequestAttributes()).getRequest();
		//사용할 변수 초기화 
		String user_id = null;//게시물작성자 ID변수
		BoardVO boardVO = null;//boardVO.getWriter()게시물의 작성자 ID를 구하려면 필요객체변수
		...
		for(Object object:pjp.getArgs()) {
			...
			if(object instanceof BoardVO) {//BoardVO 클래스형 데이터형임
				user_id = ((BoardVO) object).getWriter();
				
			}
			//조인포인트의 메서드의 매개변수가 object인데, 이 값과 instanceof 데이터형을 비교
			if(object instanceof Integer) {//bno일때는 Integer형 데이터형임
		...
		if(request != null) {
			...
			//위 메서드의 매개변수를 이용해서 생성된 user_id와 세션 user_id와 비교 + ROLE_USER일때 조건추가
			if( !user_id. equals(session.getAttribute("session_userid")) && "ROLE_USER".equals(session.getAttribute("session_levels")) ) {
				...
				//위 RedirectAttribute를 사용하는 대신 Flash(휘발성객체1번 사용하고 내용없어짐)클래스
	...
	//아래 조인포인트(중단점)은 검색어와 게시판 타입의 값을 세션으로 유지시키는 기능
	...
	//이 메서드는 컨트롤러에서 Exception이 발생했을때 여기서 인터셉처(가로채기)해서 에러 메세지를 개발자가 작성한 jsp화면에 뿌려주는 기능을 추가 prevPage 변수1, exception 변수2전송함
	...
	public ModelAndView errorModelAndView(Exception ex, HttpServletRequest request) {
		...
		request.getSession().setAttribute("session_prevPage", referer);//prevPage 세션변수 만듭니다
		
		//----------------------------------------------------
		//컨트롤러에서 받은 Exception을 ModelAndView로 전달(아래)
		modelAndView.addObject("exception", ex);
		modelAndView.setViewName("home/error/error_spring");//return String 
		return modelAndView;//.jsp생략
	}
