...
private void calcPage() {
	...
	//2 1.9 1.8 ....1.2 1.1 =>2
	//ceil(11/10)*10 => 20페이지 == tempEnd 1~10페이지에서 11페이지 값이 존재하면, 끝페이지에 임시로 20이라는 숫자를 줌
	
	int tempEnd = (int) Math.ceil(page/(double)this.perPageNum)*this.perPageNum; 
	//jsp에서 클릭한 페이지번호 예로 1부터 10까지는 클릭하면, 임시 끝페이지가 10
	//만약에 11페이지를 클릭하면, 임시 끝페이지가 20. 확인 위 temEnd변수값으로 아래내용에 이용  시작페이지를 계산하게 됩니다.
	this.startPage = (tempEnd - this.perPageNum) + 1;//ui페이지 하단에 페이지번호가 출력되도록하는 반복문의 시작 변수.
	//예, 1-10까지는 page를 jsp에서 클릭했을때, 시작페이지가 1페이지, 하지만, 11-20부터는 시작페이지가 위 계산식을 이용하면
	//11페이지로 됩니다.
}
...
private int perPageNum;//UI 하단에 보여줄 페이징 개수 계산
   private int totalCount;//계산식의 기초값으로 이 전체개수가 구해진 이후 페이징계산식이 진행됩니다
