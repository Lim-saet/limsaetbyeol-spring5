...
public class LoginController {
	...
	public String login_success(HttpServletRequest request, RedirectAttributes rdat) throws Exception {
		...
		//rdat의 목적: model객체로 값을 전송할 수 없는 상황일때, 값은 jsp로 전송하기 위해서 
		Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
		//싱글톤 객체를 만들거나 사용하는 목적: 메모리관리를 위해서 1회만 객체로 만들어서 사용하기 위해서(new로 객체를 생성하지 못함)
		String userid = "";//사용자ID
		String levels = "";//권한이 들어갈 변수
		Boolean enabled = false;//로그인체크가 들어가 변수 (true=아이디/암호 비교성공)
		//pricipal 객체는 UserDetails 객체가 추출되고  enabled라는 인증결과가 존재합니다 
		...
		if(enabled) {
			...
			session.setAttribute("session_levels", levels);//로그인한 회원의 권한확인
			session.setAttribute("session_userud", userid);//로그인한 아이디를 출력
			MemberVO memberVO = memberService.readMember(userid);
			session.setAttribute("session_username", memberVO);
		}
