...
/**
 * 이 클래스는 Admin관리자단을 접근하는 클래스<- 디스패쳐 서블렛(게이트웨이) 기능을 합니다
 * 디스패쳐 서블렛 클래스는 톰캣이 실행(web.xml) 될때 제일 먼저 실행되는 클래스, 그래서 게이트웨이라고 합니다
 * 디스패쳐 서블릿 실행될때, 컨트롤러의 Request매핑경로를 재등록합니다
...
public class AdminController {
	...
	public String board_insert(@RequestParam("file")MultipartFile[] files,BoardVO boardVO) throws Exception {
		//위 메서드의 BoardVO boardVO 파싱=> 내부작동은 다음처럼 됌 @RequestParam("title")String title,@RequestParam("content") String content, @RequestParam("writer") String writer....
		...
		//신규등록 jsp폼에서 보내온 boardVO값에 아래 file에 대한 임시 변수값을 저장하는 로직
		boardVO.setSave_file_names(save_file_names);
		boardVO.setReal_file_names(real_file_names);
		boardService.insertBoard(boardVO);//DB에 저장하는 서비스 호출(실행)
		return "redirect:/admin/board/board_list";//게시판 테러방지용 redirect사용(새로고침시 무한등록 방지)
		//게시판 신규등록시 자동으로 page 1로 이동됩니다
	}
	//게시물 등록 폼을 Get으로 호출합니다
	@RequestMapping(value="/admin/board/board_insert_form", method=RequestMethod.GET)
	public String board_insert_form(@ModelAttribute("pageVO")PageVO pageVO) 
			throws Exception {
	...
	public String board_update(@RequestParam("file")MultipartFile[] files, BoardVO boardVO, PageVO pageVO) throws Exception {
		...
		//기존 등록된 첨부파일 목록 구하기 List객체의 크기는 .size() 구함 기존파일이 있을때 사용
		...
		for(MultipartFile file:files) { //files[0] files[1]
			if(file.getOriginalFilename() != "") {//전송된 첨부파일명이 있다면 실행
				...
				//아래 for 목적: jsp폼에서 기존에 1번위치에 기존 파일이 있으면, 기존파일을 지우고, 신규파일을 덮어쓰는 로직 
		...
		//첨부파일 작업전, 시큐어 코딩: 입력/수정시 시큐어코딩적용ㅇ,  뷰화면에서 시큐어x
		
		String queryString = "bno="+boardVO.getBno()+"&page="+pageVO.getPage()+"&search_type="+pageVO.getSearch_type();
		return "redirect:/admin/board/board_view?"+queryString;//수정한 이후에는 board_view페이지로 이동: 새로고침 방지하기 위해서 redirect사용
	}
	...
	public String board_update_form(Model model, @RequestParam("bno")Integer bno, @ModelAttribute("pageVO") PageVO pageVO) throws Exception {
		//첨부파일용 save_file_names, real_file_names 2개의 배열값을 구해서 boardVO입력이 필요
	...
	@RequestMapping(value="/admin/board/board_delete", method=RequestMethod.POST)
		...
		//DB테이블삭제한 이후, 첨부파일부터 있으면 삭제처리 후  자바에서 파일핸들링처리
		//기존 등록된 첨부파일 폴더에서 삭제할 UUID(고유한 식별값생성클래스)이름을 추출합니다(아래)
	...
	//게시물 상세보기 폼으로 접근하지 않고 URl쿼리 스트링으로 접근
	...
	public String board_view(@RequestParam("bno")Integer bno,@ModelAttribute("pageVO")PageVO pageVO, Model model) throws Exception {
		...
		//첨부파일 부분 데이터도 board_view.jsp로 이동해야함(아래)
		...
		//attach테이블안의 해당 bno 게시물의 첨부파일 이름 파싱해서 jsp로 보내주는 과정(아래)
		int cnt = 0;
		...
		//위 for은 세로데이터(다수레코드)를 가로데이터(1개의 레코드,배열)에 담아서 1개레코드 boardVO로 만드는게 목적
		...
		//업로드한 파일이 이미지인지 아닌지 확인하는 용도의 데이터입니다(목적: 이미지일때 미리보기 img태그를 사용하기 위해서)
	...
	public String board_list(@ModelAttribute("pageVO")PageVO pageVO, Model model) throws Exception {
		...
		pageVO.setQueryPerPageNum(5);//토탈 카운트를 구하기전 2개의 값이 필수로 필요
	...
	//jsp에서 게시판 생성관리에 Get/Post 접근할 때 URL을 bbs_type으로 지정합니다
	//왜 board_type으로 하지않고, bbs_type하는 이유는 왼쪽메뉴 고정시키는 로직에서 경로 board와 겹치지 않도록
	...
	public String selectBoardTypeList(Model model) throws Exception {//목록폼1
		//아래 모델은 AOP기능 중 ControllerAdvice 인터페이스로 구현했기 때문에 아해는 실행인힘
	...
	//bbs_type_insert.jsp의 입력폼에서 전송된 값을 BoardTypeVO에 자동으로 담겨서 {구현} 단, 자동으로 값이 바인딩 되려면 폼의 name과 VO 멤버변수명이 동일해야함 
	...
	public String insertBoardType(BoardTypeVO boardTypeVO) throws Exception {//입력처리1
		...
		//리다이렉트(뒤로가기 불가능)는 절대경로, forward: 이동이 가능(뒤로가기 가능)
		//쇼핑몰에서 결제화면을 처리 후 뒤로가기를 누르면, 리다이렉트는 데이터가 사라지기때문에 재결제가 불가
		//forward로 결제화면을 처리 후 뒤로가기를 누르면, 재결제가 발생됩니다 이러면 안되기때문에 사용안함
	}
	//게시판 생성관리는 이 기능은 사용자단에서 UI를 사용할 일이 없기 때문에 Read, Update 1개로 사용
	@RequestMapping(value="/admin/bbs_type/bbs_type_update", method=RequestMethod.GET)
	public String updateBoardForm(@RequestParam("board_type")String board_type, Model model) throws Exception {//수정폼1
		model.addAttribute("boardTypeVO", boardTypeService.readBoardType(board_type));//서식 model.addAttribute("jsp변수", "데이터객체")
		//서식model.add~("jsp변수로 담아서 view화면으로 보냄","서비스에서 쿼리실행한 데이터객체");
	...
	public String deleteBoardType(@RequestParam("board_type")String board_type) throws Exception {//삭제처리1
		...
		return "redirect:/admin/bbs_type/bbs_type_list";//.jsp 생략
	}
	//아래 경로는 회원 신규등록 폼을 호출하는 URL쿼리스트링으로 보낸 것을 받을때는 GET방식으로 받습니다
	//========================================================================
	...
	public String insertMember(HttpServletRequest request, MultipartFile file, PageVO pageVO,MemberVO memberVO) throws Exception {
		...
		String rawPassword = memberVO.getUser_pw();//원시패스워드값
		String encPassword = passwordEncoder.encode(rawPassword);
		memberVO.setUser_pw(encPassword);
		memberService.insertMember(memberVO);
		return "redirect:/admin/member/member_list";//.jsp로 생략
	}
 ...
 {   // 페이지진입시 받은 클래스 변수값 PageVO pageVO
	...
	// URL 요청경로는 @RequestMapping 반드시 절대경로로 표시 //개발자 A 작업
	...
	//아래, <c:import방식 : 최신 게시물용도로 사용/ 페이지 안에서 컴파일된 다른 페이지를 불러올 수 있음//개발자 B작업
	...
	public String latest_board(@RequestParam(value="board_name",required=false) String board_name, @RequestParam(value="board_type",required=false) String board_type,Model model) throws Exception {
		...
		return "admin/latest/latest_board";//.jsp 생략, 최신게시물을 출력하는 결과페이지 생성
	}
}
