...
public class CommonUtil {
	...
	//사용자단에서 사용: JsonView방식으로 RestApi 구현실습
	...
	public String id_check_2010(@RequestParam("user_id")String user_id,Model model) throws Exception {
		String memberCnt = "1";//중복 ID가 있는 것을 기본값으로 지정
		if(!user_id.isEmpty()) {
			MemberVO memberVO = memberService.readMember(user_id);
	...
	public void profile_upload(String user_id, HttpServletRequest request, MultipartFile file) throws IOException {
		...
		String folderPath = request.getServletContext().getRealPath("/resources/profile");//url로 업로드할수 없기때문에 서버경로를 구하는 명령
		File makeFolder = new File(folderPath); //공백인 객체가 생성
		if(!makeFolder.exists()) {
			makeFolder.mkdir();//여기서 신규 폴더가 생성됩니다
		}
		
		//1개에 파일이 1000개 이상이면 조회속도가 엄청느려짐
		//년월폴더를 생성 후 해당년월에 업로드된 파일은 년월폴더로 관리
		byte[] fileData = file.getBytes();
		File target = new File(makeFolder,user_id+".jpg");//user_id는 PK이기 때문에 기존파일이 있다면 덮어쓰면서 저장 됩니다
		FileCopyUtils.copy(fileData, target);//첨부파일이 업로드폴더에 저장됌
	}
